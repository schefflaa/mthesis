...
class LitBaseModel(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        x_img, x_features, y, y_ts, x_ts = batch
        y_pred = self.model(x_img, x_features)
        ...
        with torch.no_grad(): # Rescale the predictions and target values for debugging and viz
            ...
            if self.verbose and loss_wh > self.loss_outlier_threshold:
                visualize_batch((x_img, x_features, y_true_rescaled, y_ts, x_ts), y_pred_rescaled, batch_idx, self.trainer.current_epoch, self.trainer.global_step, self.logger.log_dir, loss_wh)
    ...
    def batch_eval(self, batch, batch_idx, mode=None):
        ...
        for fhi in range(y_pred_clamped.shape[1]):
            os.makedirs(os.path.join(self.logger.log_dir, "preds", f'fh{fhi}'), exist_ok=True) # Create the directory if it does not exist, if it does, do nothing
            y_pred_clamped[:, fhi] = torch.clamp(y_pred_clamped[:, fhi], min=0)
            predictions_path = os.path.join(self.logger.log_dir, "preds", f'fh{fhi}', f'{mode}_pred_b{batch_idx}.csv')
            pd.DataFrame({
                "timestamp":    y_ts[:, fhi].detach().cpu().numpy(),
                "y_true":       y_true_rescaled[:, fhi].detach().cpu().numpy(),
                "y_pred":       y_pred_clamped[:, fhi].detach().cpu().numpy(),
                "batch":        batch_idx
    ...
    def on_validation_epoch_end(self):
        ...
        if not self.trainer.sanity_checking:
            val_total_rmse, val_total_mae = [], []
            for fhi in range(self.trainer.datamodule.config["f_horizon"]+1):
                metrics = self.full_eval(mode="val", epoch=self.trainer.current_epoch, fhi=fhi)
                for name, value in metrics.items():
                #     self.log(f'val/fh{fhi}/{name}', value)
                #     self.log(f'hp/val/fh{fhi}/{name}', value)

                    if name == "total_rmse":
                        val_total_rmse.append(value)
                    elif name == "total_mae":
                        val_total_mae.append(value)
        
            # Log the average RMSE and MAE for all horizons
            self.log('hp/valmeantotal_rmse', np.mean(val_total_rmse), prog_bar=True)
            self.log('hp/valmeantotal_mae', np.mean(val_total_mae))
    ...
    def on_test_end(self):
        ...
        """
        for fhi in range(self.trainer.datamodule.config["f_horizon"]+1):
            metrics = self.full_eval(mode="test", fhi=fhi)
            for name, value in metrics.items():
                self.logger.experiment.add_scalar(f'test/fh{fhi}/{name}', value, global_step=self.global_step)


    def full_eval(self, mode:str, epoch:str="", fhi:int=0) -> dict:
        """ 
        ...
        """
        predictions_path = os.path.join(self.logger.log_dir, "preds", f'fh{fhi}')
...
def visualize_batch(batch: tuple, pred, batchidx: int, epoch: str, step:int, logdir: str, loss: float) -> None:
    ...
    for i in range(batch_size):
        ...
        ax.axis('off')
        ytime = f'{pd.to_datetime(y_ts[i][0], unit="s", utc=True).tz_convert("Europe/Zurich")}'   
        ytime = "\n".join(ytime.split(" "))
        ax.text(
            0.5, 0.5,
            f'Sequence {i}\n {ytime}',
