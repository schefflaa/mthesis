...
class RsMultiplePVModel(nn.Module):
    ...
    def forward(self, x_img, pv_output_history):
        ...
        lstm_out = self.lstm(lstm_in, pv_output_history)                          
...
class RsLSTM(nn.Module):
    def __init__(
        ...
        self.lstm2 = nn.LSTM(
            input_size  = intermediate_dim +1, # +1 is for the pv_output_history (1 feature per time step)
            hidden_size = out_dim, 
            batch_first = True, 
            num_layers  = num_layers,
            dropout     = cell_dropout
    ...
    def forward(self, x, pv_output_history):
        h1 = self.init_hidden(len(x), self.hidden_dim)
        h2 = self.init_hidden(len(x), self.hidden_dim_out)

        # x.shape = [batch, seq, feature]
        lstm1_out, (zt1, ct1) = self.lstm1(x, h1) # -> out1.shape = [batch, seq, intermediate_dim]
        lstm2_in = torch.cat([lstm1_out, pv_output_history], dim=-1) # adds an additional feature to the input of the second lstm
        lstm2_out, (zt2, ct2) = self.lstm2(lstm2_in, h2)        # lstm2_in.shape = [batch, seq, intermediate_dim+1]
        return lstm2_out
