...
class RsModel(nn.Module):
    def __init__(self, img_size=(3, 250, 250), lstm_out_size=64, num_conv_blocks=2, final_actfn=torch.nn.Identity(), conv_init=None, lstm_init=None, fc_init=None):
        super().__init__()
    ...
    def init_weights(self):
        ...
        elif self.fc_init == "kaiming_normal": 
            nn.init.kaiming_normal_(self.fc.weight)            
        elif self.fc_init == "kaiming_uniform": 
            nn.init.kaiming_uniform_(self.fc.weight)
        elif self.fc_init == "xavier_normal": 
            nn.init.xavier_normal_(self.fc.weight)
        elif self.fc_init == "xavier_uniform":
...
class RsLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # the input tensors should be provided as (batch, seq, feature)
        out, (zt, ct) = self.lstm1(x, h1)
        out, (zt, ct) = self.lstm2(out, h2)
        return out
