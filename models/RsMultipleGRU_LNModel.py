...
class RsGRU(nn.Module):
    def __init__(
        ...
        self.layer_norm = nn.LayerNorm(intermediate_dim)  # Still using Layer Normalization between GRUs
        self.gru2 = nn.GRU(
    ...
    def forward(self, x):
        ...
        gru1_out, h1_out = self.gru1(x, h1)  # gru1_out.shape = [batch, seq, intermediate_dim]
        gru1_out = self.layer_norm(gru1_out)  # Applying Layer Normalization to gru1 outputs
        gru2_out, h2_out = self.gru2(gru1_out, h2)  # gru2_out.shape = [batch, seq, out_dim]
        return gru2_out
