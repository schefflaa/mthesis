...
class ExtraLstmTeacherForcing(nn.Module):
    def __init__(self, img_size=(3, 250, 250), locations=None, lstm_out_size=64, num_conv_blocks=2, pred_amount=1, final_actfn=torch.nn.Identity(), conv_init=None, lstm_init=None, fc_init=None, teacher_forcing_ratio=0.5):
        super().__init__()
        ...
        self.decoder_lstm = nn.LSTM(input_size=1, hidden_size=lstm_out_size, batch_first=True)  # 1 = dim of scalar target
        self.output_layer = nn.Linear(lstm_out_size, 1)
        self.activation_fn = final_actfn


    def forward(self, x_img, targets):
        ...
        B, seq_len, L, C, H, W = x_img.shape
        bat_len = x_img.shape[0]
        seq_len = x_img.shape[1]
        
        # CNN
        cnn_in = x_img.view(-1, C, H, W)                  # reshape input to [B*seq*L, C, H, W] for the CNN
        cnn_out  = self.cnn(cnn_in)              

        # LSTM
        lstm_in = cnn_out.view(B, seq_len, -1)            # reshape the CNN output to [B, seq, L*C*H*W] for the LSTM
        lstm_out, (zt, ct) = self.lstm(lstm_in)           # lstm_out.shape = [B, seq, out_dim]

        # Decoder (generate pred_amount steps)
        decoder_input = torch.zeros(B, 1, 1).to(x_img.device)
        outputs = []

        for t in range(self.pred_amount):
            out, (zt, ct) = self.decoder_lstm(decoder_input, (zt, ct))  # out: [B, 1, hidden_dim]
            pred = self.output_layer(out.squeeze(1))                # pred: [B, 1]
            pred = self.activation_fn(pred)
            outputs.append(pred)

            # Determine next decoder input (teacher forcing)
            if self.training and targets is not None and torch.rand(1).item() < 0.5:
                next_input = targets[:, t].unsqueeze(1).unsqueeze(2)  # [B, 1, 1]
            else:
                next_input = pred.unsqueeze(1)  # [B, 1, 1]
            decoder_input = next_input

        return torch.cat(outputs, dim=1)  # [B, pred_amount]
