...
class OmniModel(nn.Module):
    def __init__(self, img_size=(3, 250, 250), locations=None, pred_amount=1, lstm_out_size=64, num_conv_blocks=2, final_actfn=torch.nn.Identity(), conv_init=None, lstm_init=None, fc_init=None):
        super().__init__()
    ...
    def forward(self, x_img, sunposition):
        ...
        lstm_out = self.lstm(lstm_in, sunposition)                          
...
class RsLateCombLSTM(nn.Module):
    def __init__(
        ...
        self.lstm = nn.LSTM(
            input_size  = intermediate_dim*self.strands +2,
            hidden_size = out_dim, 
            batch_first = True, 
            num_layers  = num_layers,
            dropout     = cell_dropout
    ...
    def forward(self, x, sunposition):
        ...
        lstm2_in = torch.cat([lstm1_out, sunposition], dim=-1)      # adds an additional feature to the input of the second lstm
        lstm2_out, (zt2, ct2) = self.lstm(lstm2_in, h2)
        return lstm2_out
