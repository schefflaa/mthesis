...
# ------------------------------ Model definition --------------------------------
class PatchMeanBrightnessLRModel(nn.Module):
    def __init__(self, input_size, patch_size):
        ...
        super(PatchMeanBrightnessLRModel, self).__init__()
        ...
        num_patches = (input_size // patch_size) ** 2 # Total number of patches
    ...
    def forward(self, x):
        ...
        # Apply average pooling to compute mean brightness of patches
        ...
        if not IS_TRAIN:
            ...
            cv2.imwrite(f"viz/gray{self.patch_size}original.png", img)
            ...
            cv2.imwrite(f"viz/gray{self.patch_size}patch_means.png", m)
...
class LitPMBLRModel(LitBaseModel):
    def initialize_model(self, **kwargs):
        return PatchMeanBrightnessLRModel(**kwargs)
...
logger = TensorBoardLogger(save_dir="baselines/tblogs", name="PatchBrightnessLR", version=RUN_NAME, default_hp_metric=False)
hparams = {
